...
df <- read.csv("final_data", stringsAsFactors = FALSE)
...
df <- df %>%
  mutate(
    ...
    Social_Task_Median_RT_TOM_Adj = Social_Task_Median_RT_TOM - WM_Task_0bk_Median_RT,
    ER40_CRT_Adj = ER40_CRT - WM_Task_0bk_Median_RT
  ) %>%
  # Keep only the adjusted columns and retain Subject column
  select(Emotion_Task_Face_Median_RT_Adj, Language_Task_Story_Median_RT_Adj, Social_Task_Median_RT_TOM_Adj, ER40_CRT_Adj) %>%
...
```{r}
# Define CFA model to test our EFA hypothesis
cfa_model <- '
  # Factor 1 (ML1)
  ML1 =~  Emotion_Task_Face_Median_RT_Adj+ Language_Task_Story_Median_RT_Adj+ Social_Task_Median_RT_TOM_Adj+ ER40_CRT_Adj
