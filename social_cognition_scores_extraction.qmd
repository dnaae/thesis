...
execute_analysis <- function(file_path, model_type = NULL) {
  ...
  df <- read.csv(file_path, stringsAsFactors = FALSE)
  df$Subject <- factor(df$Subject)
  # Adjust ER40_CR if the model type is "accuracies"
  ...
  if (model_type == "questionnaire") {
    # Invert variables for the questionnaire model
  ...
  # Define CFA model based on the model type
  if (model_type == "w0bk") {
...
sc_density_histogram_plot <- ggplot(social_scores, aes(x = .data[[social]])) +
  ...
  # Add a normal distribution curve using the data's mean and standard deviation
  stat_function(
    fun = dnorm, 
